...
/* Static datasets used to build the interface */
const SKIN_OPTS = [
...
let concernSwitchTimer = null;
let neutralCrossfadeTimer = null;
...
function filterText(value) {
  if (value === 'Fragrance') return 'Fragrance excluded';
  if (value === 'Alcohol') return 'Alcohol excluded';
  if (value === 'Both') return 'Fragrance and alcohol excluded';
  return 'No ingredient filter';
}
...
function normalizeVisibleProduct(image) {
  if (!image.src.includes('/query-products-transparent/')) {
    return;
  }
...
function createProductImage(name, imageValue) {
  ...
  const candidates = [
    `./images/query-products-transparent/${encodeURIComponent(name)}.png`
  ];

  if (LOCAL_IMAGES[name]) {
    candidates.push(LOCAL_IMAGES[name]);
  }

  if (safeHttp(imageValue)) {
...
/* Query builder for the SPARQL service */
function buildQuery(condition, allergenValue) {
...
function radioKeys(event, group) {
  ...
  if (group === 'allergen') {
    selectAllergen(buttons[index].dataset.value);
  }
}
...
function updateOnboarding() {
  ...
  selectionFilter.textContent = filterText(allergen.value);
...
function showNeutralConcern() {
  ...
  [concernMainImage, concernNextImage].forEach(layer => {
    layer.className = '';
  });
  playNeutralConcern();
  syncOptions();
...
function pauseNeutralConcern() {
  ...
  if (neutralCrossfading) {
    neutralVideoIndex = 1 - neutralVideoIndex;
  }

  concernNeutralVideos.forEach((video, index) => {
    video.pause();
    if (index !== neutralVideoIndex) {
      video.currentTime = 0;
    }
...
function addProductInfo(parent, binding, condition, allergenValue, headingClass = '') {
  ...
  if (!headingClass && productName.length > 46) {
    name.classList.add('is-long-title');
  }
...
function renderResults(bindings, condition, allergenValue) {
  ...
  if (mobile || reducedMotion || featured.length < 2) {
  ...
  stickyFrame.append(results.querySelector('.results-header'), resultsMeta, sticky);
  ...
  const goTo = index => {
    ...
    if (lenis) {
      lenis.scrollTo(target, { duration: .72 });
    } else {
...
function setSearchBusy(busy) {
  ...
  [...skinProblemOpts.querySelectorAll('button'), ...allergenOpts.querySelectorAll('button'), clearAllergen, resetChoicesButton].forEach(button => {
...
function setupScrolling() {
  ...
  if (!reducedMotion && !mobile && window.Lenis) {
    try {
      lenis = new Lenis({
        duration: 1.05,
        smoothWheel: true,
...
function setupNeutralVideoLoop() {
  ...
  concernNeutralVideos.forEach(video => {
...
function setupPremiumMotion() {
  ...
  const sectionObserver = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      videoSection.classList.toggle('is-visible', entry.isIntersecting);
    });
  }, { threshold: .08 });
